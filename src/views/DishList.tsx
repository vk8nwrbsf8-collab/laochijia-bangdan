...
import { Search, Share, MapPin, User as UserIcon, Utensils } from 'lucide-react';
...
export default function DishList() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach(doc => {
        data.push({ id: doc.id, ...doc.data() } as Dish);
      });
      setDishes(data);
      setLoading(false);
    }, (err) => {
  ...
  useEffect(() => {
    ...
    fetchAllCities();
  }, [user, dishes]); // re-run when dishes update so it captures newly added dish cities

  const categories = ['全部', ...Array.from(new Set(dishes.map(s => s.category)))];
  const availableCities = allCities.length > 0 ? allCities : Array.from(new Set(dishes.map(s => s.city).filter(Boolean)));

  // No auto-select: default is "全部" (show all cities)

  const filteredDishes = dishes.filter(s => {
    ...
    <div className="flex flex-col h-full bg-secondary relative">
      ...
      <div className="bg-primary px-4 py-3 flex items-center justify-between z-10 border-b border-theme shrink-0">
        ...
        <div className="flex items-center gap-4 text-tertiary">
          ...
          <button onClick={() => navigate('/poster?type=dish')} className="active:scale-95 transition-transform"><Share className="w-5 h-5" /></button>
        </div>
      </div>
